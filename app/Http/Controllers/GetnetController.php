...
class GetnetController extends Controller
{
    ...
    public function TokenGenerateEventos(){
        try {
            //code...
    ...
    public function CardTokenizerEventos($card){
        try {
            ...
            $response = $client->post( env('GETNET_URL_API').'/v1/tokens/card',
                [
                    'headers' => [
                        'authorization' => 'Bearer '.$this->TokenGenerateEventos()->access_token,
                        'content-type'=>'application/json; charset=utf-8',
                    ],
                    'json' => [
                        'card_number' => $card
    ...
    public function CredPayment(Request $request)
    {
        ...
        try {
            ...
            if(Auth::user()->id == $carrinho->user_id){                
                ...
                foreach ($carrinho->ItensCarrinho()->get() as $i) {
                    ...
                    if($i->ExtAtvTurma->ExtAtvVagas($i->ExtAtvTurma->id)>0 && empty($lista)){
                        if($i->ExtAtvTurma->ExtAtv->terceirizada==1)
                            $amount_t += floatval(str_replace(',','.',$i->ExtAtvTurma->valor));
                        else
                ...
                $amount_t = str_replace('.','',number_format($amount_t, 2, '.', ''));
                //dd($amount,$amount_t);
                if($amount > 0){
                    $client = new \GuzzleHttp\Client();                
                    $response = $client->post(env('GETNET_URL_API').'/v1/payments/credit',
                        ...
                        foreach ($carrinho->ItensCarrinho()->get() as $i) {
                            $lista = ExtAtvListaDeEspera::where('ext_atv_turmas_id',$i->ExtAtvTurma->id)->where('ano',date('Y'))->first();
                            if($i->ExtAtvTurma->ExtAtvVagas($i->ExtAtvTurma->id)>0 && empty($lista) && $i->ExtAtvTurma->ExtAtv->terceirizada ==0){
                ...
                if($amount_t > 0){
                    ...
                    $response = $client->post(env('GETNET_URL_API_EVENTOS').'/v1/payments/credit',
                        [
                            ...
                            'json' => [
                                ...
                                'credit' => [
                                    ...
                                    //'dynamic_mcc'=> 1799,
                                    'card'=>[
                                        'number_token' => $this->CardTokenizerEventos($request->numero)->number_token,
                                        'cardholder_name'=> $request->nome,
                                        'expiration_month'=> $request->mes,
                                        'expiration_year' => $request->ano,
                                        'security_code' => $request->cod,
                        ...
                        foreach ($carrinho->ItensCarrinho()->get() as $i) {
                            $lista = ExtAtvListaDeEspera::where('ext_atv_turmas_id',$i->ExtAtvTurma->id)->where('ano',date('Y'))->first();
                            if($i->ExtAtvTurma->ExtAtvVagas($i->ExtAtvTurma->id)>0 && empty($lista) && $i->ExtAtvTurma->ExtAtv->terceirizada ==1){
                                $inscricao = new ExtInscricao();
                                $inscricao->aluno_id = $carrinho->aluno_id;
                                $inscricao->ano = date('Y');
                                $inscricao->amount = str_replace('.','',number_format(str_replace(',','.',$i->ExtAtvTurma->valor), 2, '.', ''));
            ...
            //$e->getRequest()
            dd($e,$e->getResponse(),$e->getResponse()->getBody());
            $e = json_decode($e->getResponse(),true);
            $e = json_decode($e->getResponse()->getBody(),true);
            //dd($error);
            return view('errors.error', compact('e'));
